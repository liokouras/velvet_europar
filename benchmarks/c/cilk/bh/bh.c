...
void tree_traverse_update_par(const Tree *restrict tree, const Tree *restrict root, Body *restrict bodies) {
    if (tree->body.type == TREE_LEAF) {
        ...
        cilk_spawn tree_traverse_update_par(tree->body.aggregate.children[0], root, bodies);
        cilk_spawn tree_traverse_update_par(tree->body.aggregate.children[1], root, bodies);
        cilk_spawn tree_traverse_update_par(tree->body.aggregate.children[2], root, bodies);
