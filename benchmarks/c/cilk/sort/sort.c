...
int merge_cilk(int32_t *left_low, int32_t *left_high, int32_t *right_low, int32_t *right_high, int32_t *dest_low) {
    // want 'left' to be the larger of the two input arrays
    if (right_high - right_low > left_high - left_low) {
        swap_indices(left_low, right_low);
        swap_indices(left_high, right_high);
    ...
    cilk_spawn merge_cilk(left_low, left_mid, right_low, right_mid, dest_low);
    merge_cilk(left_mid, left_high, right_mid, right_high, dest_low + dest_offset);
    cilk_sync;
...
int sort_cilk(int32_t *arr, int32_t *buf, int len, int usebuf) {
    if (len < DIRECT_THRESHOLD && usebuf == 1) {
        rust_sort_i32(arr, len);
    } else {
        int mid = len/2 + 1;
        cilk_spawn sort_cilk(arr, buf, mid, 1 - usebuf);
        sort_cilk(arr+mid, buf+mid, len-mid, 1 - usebuf);
        cilk_sync;
        ...
        merge_cilk(left_low, left_high, right_low, right_high, dest_low);
    }
    return 0;
}
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf (stderr, "Usage: %s [cilk|seq] [number_of_elements] [random_seed]\n", argv[0]);
        return 1;
    }
    ...
    if (strcmp(app, "cilk") == 0) {
        ...
        ctimer_start(&t);
        sort_cilk(arr, buf, n, 1);
        ctimer_stop(&t);
        ctimer_measure(&t);

        printf("12,%d,%d,%d,%d,%ld.%09ld\n", workers,  n, seed, DIRECT_THRESHOLD, (long)t.elapsed.tv_sec, t.elapsed.tv_nsec);
        
        fprintf(stderr, "CILK sort(%d %d), threshold = %d, num workers = %d\n", n, seed, DIRECT_THRESHOLD, workers);
        check(arr, n);
        ctimer_print(t, "sort_cilk");
