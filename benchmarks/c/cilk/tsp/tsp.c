...
void tsp_cilk(int hops, int last, uint128 path, int length) {
    const int ntowns = distance_num_towns(DISTANCE);
    ...
    for (int i = 0; i < ntowns; i++) {
        ...
        if (city != last && (path & city_bit) == 0) {
            const int dist = DISTANCE->dist[last * ntowns + i];            
            const uint128 new_path = path | city_bit;
            cilk_spawn tsp_cilk(hops + 1, city, new_path, length + dist);
        }
    }
    cilk_sync;
}
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf (stderr, "Usage: %s [cilk|seq] [number_of_towns] [random_seed]\n", argv[0]);
        return 1;
    }
    ...
    if (strcmp(app, "cilk") == 0) {
        ...
        fprintf(stderr, "CILK tsp(%d %d) = %d, threshold = %d, num workers = %d\n", ntowns, seed, result, THRESHOLD, workers);
        ctimer_print(t, "tsp_cilk");
