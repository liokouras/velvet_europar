...
void matmul_par(int task, Matrix *c, Matrix *a, Matrix *b) {
    ...
    switch (a->kind) {
        case INTERNAL:
            ...
            // recursive multiplication
            #pragma omp task
            matmul_par(task-1, c->internal.m00, a->internal.m00, b->internal.m00);

            #pragma omp task
            matmul_par(task-1, c->internal.m00, a->internal.m01, b->internal.m10);
            
            #pragma omp task
            matmul_par(task-1, c->internal.m01, a->internal.m00, b->internal.m01);

            #pragma omp task
            matmul_par(task-1, c->internal.m01, a->internal.m01, b->internal.m11);

            #pragma omp task
            matmul_par(task-1,c->internal.m10, a->internal.m10, b->internal.m00);
            
            #pragma omp task
            matmul_par(task-1,c->internal.m10, a->internal.m11, b->internal.m10);

            #pragma omp task
            matmul_par(task-1,c->internal.m11, a->internal.m10, b->internal.m01);
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf (stderr, "Usage: %s [omp|seq] [depth] [dim] \n", argv[0]);
        return 1;
    }
    ...
    if (strcmp(app, "omp") == 0) {
        ...
        printf("11,%d,%d,%d,%ld.%09ld\n", workers, depth, dim, (long)t.elapsed.tv_sec, t.elapsed.tv_nsec);
        
        fprintf(stderr, "OMP matmul(%d %d), num workers = %d\n", depth, dim, workers);
